...
namespace eos
{
    ...
    struct Implementation<BqToDqVector>
    {
            ...
            opt_accuracy(o, options, "accuracy")
        {
            ...
            if (nullptr == ff.get())
                throw InternalError("Failed to construct form factors for option '" + opt_ff.value() + "'");
        ...
        complex<double> a_1() const
        {
            ...
            /*auto a_1_nlo_integrand = [&](const double & u, double switch_TTLL) -> complex<double>
            {
                complex<double> TVLL = (-18.0 - 6.0 * 2.0 * log(mu() / mb) + fVLL(1.0 - u, 1.0 / z) + fVLL(u, z) + (3.0 + 2.0 * log(u / (1.0 - u))) * log(z * z)) * this->lcdas->phipara(u, mu());

                complex<double> TVLR = (6.0 + 6.0 * 2.0 * log(mu() / mb) - (3.0 + 2.0 * log((1.0 - u) / u)) * log(z * z) - fVLL((1.0 - u), z) - fVLL(u, 1.0 / z)) * this->lcdas->phipara(u, mu());

                complex<double> TSLL = (-4.0 * (2.0 * u - 1.0) * (1.0 - z) / (1.0 + z) * 2.0 * log(mu() / mb) + 2.0 * ((2.0 * u - 1.0) * (1.0 - z) / (1.0 + z) + log(u / (1.0 - u))) * log(z * z) + fSLL(u, z) + fSLL((1.0 - u), 1.0 / z)) * phiV(u, mu());

                complex<double> TSLR = (2.0 * log(u / (1.0 - u)) * log(z * z) - 6.0 + fSLR(u, z) + fSLR((1.0 - u), 1.0 / z)) * phiV(u, mu());

                complex<double> TTLL = switch_TTLL * (-48.0 * 2.0 * log(mu() / mb) + 8.0 * (3.0 + ((u - (1.0 - u)) * (1.0 - z)) / (1.0 + z) * log(u / (1.0 - u))) * log(z * z) + fTLL(u, z) + fTLL(1.0 - u, 1.0 / z)) * phiV(u, mu());
           ...
           /* // convoluted 3-particle hard-scattering kernels
            ...
            const complex<double> a_1_nlo =
                    ...
                    32.0 / 9.0 * (wc.c9() + wc.c9p()) * ((2.0 * mu_L * (4.0 * TSLL + TTLL)) / (mb - mc) + TVLR - 40.0) +
                    16.0 / 27.0 * (wc.c10() + wc.c10p()) * (-((2.0 * mu_L * (4.0 * TSLL + TTLL)) / (mb - mc)) - TVLR + 76.0);
            std::cerr << "a_1_lo: " << a_1_lo << std::endl;
            std::cerr << "a_1_nlo: " << a_1_nlo << std::endl;
        ...
        double decay_width() const
        {
            ...
            const complex<double> amplitude = g_fermi() / sqrt(2.0) * ckm_factor() * f_V() * ff->f_p(m_V * m_V)
