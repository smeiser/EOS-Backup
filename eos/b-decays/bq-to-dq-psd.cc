...
namespace eos
{
    ...
    struct Implementation<BqToDqPseudoscalar>
    {
        ...
        complex<double> fVLL(const double & u, const double & z) const
        {
            ...
            // evaluate fVLL chosing the correct branch for the unphysical case z = m_b / m_c > 1
            else if (z > 1.0)
            {
                return
                    2.0 * (AVLL(u, z) - AVLL(1.0 - u, z)) - (z / (1.0 - u * (1 - z * z))) -
        ...
        complex<double> a_1() const
        {
            ...
            const complex<double> a_1_lo =
                ...
                ((wc.c5() - wc.c5p()) * (-1.0 + mu_L / (mb - mc))) / 6.0 +
                (2.0  * (wc.c6() - wc.c6p()) * (-1.0 + mu_L / (mb - mc))) / 9.0 +
            ...
            auto a_1_nlo_integrand = [&](const double & u) -> complex<double>
            {
                ...
                return
                    ...
                    (16.0 * (wc.c4() - wc.c4p()) * (-19.0 - 2.0 * TVLL - ((-80.0 + TSLR) * mu_L) / (mb - mc))) / 27.0 +
                    ...
                    (2.0 * (wc.c7() - wc.c7p()) * (-16.0 + ((-112.0 + 12.0 * TSLL + TTLL) * mu_L) / (mb - mc))) / 9.0 +
                    ((wc.c8() - wc.c8p()) * (-56.0 - ((140.0 + 12.0 * TSLL + TTLL) * mu_L) / (mb - mc))) / 27.0 +
                    (32.0 * (wc.c9() - wc.c9p()) * (40.0 - TVLR - (2.0 * (-48.0 + 4.0 * TSLL + TTLL) * mu_L) / (mb - mc))) / 9.0 +
                    (16.0 * (wc.c10() - wc.c10p()) * (-76.0 + TVLR + (2.0 * (204.0 + 4.0 * TSLL + TTLL) * mu_L) / (mb - mc))) / 27.0;
            };
    ...
    double
    BqToDqPseudoscalar::re_a_1() const
    {
        return real(_imp->a_1());
    }

    double
    BqToDqPseudoscalar::im_a_1() const
    {
